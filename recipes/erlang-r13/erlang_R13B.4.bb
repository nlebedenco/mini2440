include erlang.inc
DEPENDS += "erlang-native zlib openssl"
...
do_configure() {
    ...
    SHLIB_LD='${CC}' \
    erl_xcomp_sysroot='${STAGING_DIR_HOST}${layout_exec_prefix}' \
...
def get_erlang_libs_r13(d):
    ...
    for fname in os.listdir(erlang_lib):
        ...
        if fname.startswith("erts-"): continue
...
FILES_${PN}-libs += " ${@' '.join(get_erlang_libs_r13(d))}"
